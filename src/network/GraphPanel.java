 ...
 * Refactored to meet project needs
 */
public class GraphPanel extends JComponent {
    ...
    //Color set to blue
    private static final Color color = new Color(0x1E12FF);
    private ControlPanel control = new ControlPanel();
    private static final int radius = 20;
    private List<GraphicNode> nodes = new ArrayList<GraphicNode>();
    private List<GraphicNode> selected = new ArrayList<GraphicNode>();
    private List<GraphicEdge> edges = new ArrayList<GraphicEdge>();
    ...
    private class MouseHandler extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            mousePt = e.getPoint();
            if (e.isShiftDown()) {
                GraphicNode.selectToggle(nodes, mousePt);
            } else if (e.isPopupTrigger()) {
                GraphicNode.selectOne(nodes, mousePt);
                showPopup(e);
            } else if (GraphicNode.selectOne(nodes, mousePt)) {
                selecting = false;
            } else {
                GraphicNode.selectNone(nodes);
                selecting = true;
            }
            e.getComponent().repaint();
        }
    ...
    private class MouseMotionHandler extends MouseMotionAdapter {
        ...
        public void mouseDragged(MouseEvent e) {
            if (selecting) {
                ...
                GraphicNode.selectRect(nodes, mouseRect);
            } else {
                delta.setLocation(
                    e.getX() - mousePt.x,
                    e.getY() - mousePt.y);
                GraphicNode.updatePosition(nodes, delta);
                mousePt = e.getPoint();
            }
            e.getComponent().repaint();
        }
...
private class ControlPanel extends JToolBar {
        ...
        private Action deleteEdge = new DeleteEdgeAction("Delete Edge");
    ...
    private class ConnectAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            GraphicNode.getSelected(nodes, selected);
            if (selected.size() > 1) {
                for (int i = 0; i < selected.size() - 1; ++i) {
                    GraphicNode n1 = selected.get(i);
                    GraphicNode n2 = selected.get(i + 1);
                    edges.add(new GraphicEdge(n1, n2));
                }
            }
            repaint();
        }
    ...
    private class DeleteAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ListIterator<GraphicNode> iter = nodes.listIterator();
            while (iter.hasNext()) {
        ...
        private void deleteEdges(GraphicNode n) {
            ListIterator<GraphicEdge> iter = edges.listIterator();
            while (iter.hasNext()) {
     ...
     */
    private static class GraphicNode {
